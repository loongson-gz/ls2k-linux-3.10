...
static struct resource ls_spi1_resources[]={
	[0]={
		.start      =   0x1be70000,
		.end        =   0x1be70000+6,
		.flags      =   IORESOURCE_MEM,
...
static struct platform_device ls_spi1_device={
	.name   =   "ls-spi",
	.id     =       1,
	.num_resources  =ARRAY_SIZE(ls_spi1_resources),
	.resource       =ls_spi1_resources,
};

static struct resource ls_spi0_resources[]={
	[0]={
		.start      =   0x1fe00220,
		.end        =   0x1fe00220+6,
		.flags      =   IORESOURCE_MEM,
...
static struct platform_device ls_spi0_device={
	.name   =   "ls-spi",
	.id     =       0,
	.num_resources  =ARRAY_SIZE(ls_spi0_resources),
	.resource       =ls_spi0_resources,
};

static struct spi_board_info ls_spi_devices[] = {
	{	/* DataFlash chip */
		.modalias	= "m25p80",
		.bus_num 		= 0,
		.chip_select	= 0,
 ...
 *GPIO
 */
static struct platform_device gpio_ls2h = {
	.name		= "gpio-ls2h",
	.id		= 0,
};
...
static void ls2h_early_config(void)
{
	...
	ls2h_writel(val | (1 << 5) | (1 << 4) | (0 << 14) | (1 << 26), LS2H_CHIP_CFG0_REG);
...
static void __init ls2h_device_initcall(void)
{
	...
	spi_register_board_info(ls_spi_devices, ARRAY_SIZE(ls_spi_devices));
