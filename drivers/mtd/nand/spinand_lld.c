...
static int hwoobsize;
module_param(hwoobsize, uint, 0664);

//#define NOUSED_MTD_SPINAND_ONDIEECC
/*
...
static int spinand_probe(struct spi_device *spi_nand)
{
	...
	/*
	oobsize is (writesize/32)*2
	for hwecc set oobsize to writesize/2, this is kernel default value.
	*/
...
#elif defined(CONFIG_MTD_SPINAND_SOFTBCH)
#define BCH_BUG(a...) printk(a);while(1);
...
#elif defined(CONFIG_MTD_SPINAND_SOFTECC)
	chip->ecc.mode		= NAND_ECC_SOFT;
	chip->ecc.size		= 256;
	chip->ecc.bytes		= 3;
#elif defined(CONFIG_MTD_SPINAND_SOFTNOECC)
	chip->ecc.mode	= NAND_ECC_NONE;
#else
	printk("error ecc config\n");
	return -EINVAL;
#endif
	...
	if(hwoobsize)
		mtd->oobsize = hwoobsize;

	if (nand_scan_tail(mtd))
		return -1;

#ifndef NOUSED_MTD_SPINAND_ONDIEECC
	if ((chip->ecc.bytes*mtd->writesize/chip->ecc.size) > chip->ecc.layout->eccbytes) {
		printk("invalid ecc or bch value \n");
		return -EINVAL;
	}
#endif
...
static struct spi_device *get_spinand(int devnum)
{
	struct mtd_info *mtd;
	struct nand_chip *chip;
	struct spinand_info *info;
...
static int param_set_reg(const char *val, struct kernel_param *kp)
{
	unsigned char d;
	int retval;
	struct spi_device *spi_nand;
	...
	d = simple_strtoul(val, 0, 0);

	spi_nand = get_spinand(0);
	if(!spi_nand)
		return -ENODEV;
...
static int param_get_reg(char *buffer, struct kernel_param *kp)
{
	unsigned char d;
	int retval;
	struct spi_device *spi_nand;
	...
	return sprintf(buffer, "0x%02x", d);
}

static int param_set_wen(const char *val, struct kernel_param *kp)
{
	unsigned char d;
	int retval;
	struct spi_device *spi_nand;
	...
	cmd.cmd = d?CMD_WR_ENABLE:CMD_WR_DISABLE;
	retval = spinand_cmd(spi_nand, &cmd);
	if (retval != 0) {
		dev_err(&spi_nand->dev, "error %d set otp\n", retval);
		return retval;
...
module_param(regadr, uint, 0664);
module_param_call(regval, param_set_reg, param_get_reg, NULL, 0664);
module_param_call(wen, param_set_wen, NULL, NULL, 0664);
