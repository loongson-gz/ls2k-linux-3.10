...
static int ls_sound_drv_probe(struct platform_device *pdev)
{
	...
	if((np = pdev->dev.of_node))
	{
		...
		of_property_read_string_index(np, "codec-names", 0 , &z2_dai.name);
		of_property_read_string_index(np, "codec-names", 1 , &z2_dai.stream_name);
		of_property_read_string_index(np, "codec-names", 2, &z2_dai.codec_dai_name);
		of_property_read_string_index(np, "codec-names", 3, &z2_dai.codec_name);
	}
